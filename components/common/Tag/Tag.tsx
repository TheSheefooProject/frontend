...
const Tag = (props: { [x: string]: any; text?: string; onClick?: any }) => {
  // Default prop values
  const { text = '', onClick = null, ...restProps } = props

  return (
    <span className=" my-1 inline-flex h-7 w-64 flex-row items-center  whitespace-nowrap rounded-md bg-back_2 py-0.5 pr-3 ">
      <div
        onClick={onClick}
        className="flex h-7 w-7 items-center rounded-l-md px-1 text-accent_1 hover:cursor-pointer hover:bg-red-600"
      >
